...
public class TownyCommand implements TabExecutor{
    ...
    public void handleRank(Resident resident,String[] args) throws NoPermissionException {
        if(args.length < 2){
            ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankg(resident.getPlayer()));
            return;
        }
        if(!resident.hasTown()){
        ...
        switch (args[1]){
            case "create","new" -> {
                if(args.length < 3){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankf(resident.getPlayer()));
                    return;
                }
                TownyUtil.checkTownPermission(resident,"ASTRATOWN_TOWN_RANK_CREATE");

                if(TownManager.getInstance().getRank(args[2],resident.getTown()) != null){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankalrexist(resident.getPlayer()));
                    return;
                }

                TownyUtil.createRank(args[2],resident.getTown(),UUID.randomUUID());

                ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.succreaterank(resident.getPlayer()));
            }
            case "set","give" -> {
                if(args.length < 4){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rank(resident.getPlayer()));
                    return;
                }
                TownyUtil.checkTownPermission(resident,"ASTRATOWN_TOWN_RANK_ADD");

                if(TownManager.getInstance().getRank(ConfigYML.mayorname,resident.getTown()) == TownManager.getInstance().getRank(args[3],resident.getTown())){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.transfert(resident.getPlayer()));
                    return;
                }
                if(TownManager.getInstance().getRank(ConfigYML.defaultPerm,resident.getTown()) == TownManager.getInstance().getRank(args[3],resident.getTown())){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.transfert(resident.getPlayer()));
                    return;
                }

                if(TownManager.getInstance().getRank(args[2],resident.getTown()) == null){
                    if(TownManager.getInstance().getResident(args[2]) == null){
                        ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.playeroffline(resident.getPlayer()));
                        return;
                    }
                    if(TownManager.getInstance().getResident(args[2]).isMayor()){
                        ChatUtil.sendSuccessNotification(resident,Messages.transfert(resident.getPlayer()));
                        return;
                    }
                    if(TownManager.getInstance().getResident(args[2]) == resident){
                        ChatUtil.sendSuccessNotification(resident,Messages.permissionerror(resident.getPlayer()));
                        return;
                    }
                    if(TownManager.getInstance().getResident(args[2]).getTown() == resident.getTown()) {
                        if(TownManager.getInstance().getResident(args[2]).getRankTown() == (TownManager.getInstance().getRank(args[3],resident.getTown()))){
                            return;
                        }
                        Rank rank = TownManager.getInstance().getRank(args[3],resident.getTown());
                        TownManager.getInstance().getResident(args[2]).setRankTown(rank);

                        TownManager.getInstance().getResident(args[2]).setPermissionsTown(new CopyOnWriteArrayList<>(rank.getPermissionList()));

                        ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankgivesuc(resident.getPlayer()));
                    }
                }else {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankexist(resident.getPlayer()));
                }
            }
            case "delete" ->{
                if(args.length < 3) {
                ...
                TownyUtil.checkTownPermission(resident,"ASTRATOWN_TOWN_RANK_DELETE");
                if(TownManager.getInstance().getRank(args[2],resident.getTown()) == null){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankexist(resident.getPlayer()));
                    return;
                }
                if(TownManager.getInstance().getRank(args[2],resident.getTown()) == TownManager.getInstance().getRank(ConfigYML.defaultPerm,resident.getTown()) || TownManager.getInstance().getRank(args[2],resident.getTown()) == TownManager.getInstance().getRank(ConfigYML.mayorname,resident.getTown())){
                    return;
                }
                TownyUtil.deleteRank(TownManager.getInstance().getRank(args[2],resident.getTown()));
                ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.suc(resident.getPlayer()));
            }
            case "list" -> {
                for (Rank rank : TownManager.getInstance().getTown(resident).getRanks()){
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), rank.getName());
                }
            }
            case "my" -> {
                Rank ranks = resident.getRankTown();
                if (ranks == (null)) {
                    return;
                }
                ChatUtil.sendSuccessNotification(resident,ranks.getName());
            }
            default -> ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.commandundefined(resident.getPlayer()));
        }
    }
