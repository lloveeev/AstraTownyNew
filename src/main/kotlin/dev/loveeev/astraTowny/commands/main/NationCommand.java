...
public class NationCommand implements TabExecutor {
    ...
    /*
    public void rank(Resident resident,String[] args) {
        if (args.length < 2) {
        ...
        switch (args[1]) {
            case "create", "new" -> {
                ...
                TownyUtil.checkPermission(resident,"ASTRATOWN_NATION_RANK_CREATE");
                if (TownManager.getInstance().getRank(args[2], resident.getNation()) != null) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankalrexist(resident.getPlayer()));
                    return;
                }
                TownyUtil.createRank(args[2],resident.getNation(),UUID.randomUUID());
            ...
            case "add" -> {
                ...
                if (TownManager.getInstance().getRank(ConfigYML.kingname, resident.getNation()) == TownManager.getInstance().getRank(args[3], resident.getNation())) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.transfern(resident.getPlayer()));
                    return;
                }
                if (TownManager.getInstance().getRank(ConfigYML.defaultnationname, resident.getNation()) == TownManager.getInstance().getRank(args[3], resident.getNation())) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.transfern(resident.getPlayer()));
                    return;
                }
                TownyUtil.checkPermission(resident,"ASTRATOWN_NATION_RANK_ADD");
                if (TownManager.getInstance().getRank(args[2], resident.getNation()) == null) {
                    if (TownManager.getInstance().getResident(args[2]) == null || TownManager.getInstance().getResident(args[2]).getNation() == null) {
                        ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.playeroffline(resident.getPlayer()));
                        return;
                    }
                    if (TownManager.getInstance().getResident(args[2]).getNation() == resident.getNation()) {
                        if (TownManager.getInstance().getResident(args[2]).getRankNation() == (TownManager.getInstance().getRank(args[3], resident.getNation()))) {
                            return;
                        }
                        Rank rank = TownManager.getInstance().getRank(args[3], resident.getNation());
                        TownManager.getInstance().getResident(args[2]).addRankNation(rank);

                        for (String permission : rank.getPermissionList()) {
                            TownManager.getInstance().getResident(args[2]).addperm(permission);
                        }
            ...
            case "remove" -> {
                ...
                if (!resident.hasPerm("ASTRATOWN_NATION_RANK_REMOVE")) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.permissionerror(resident.getPlayer()));
                    return;
                }
                if (TownManager.getInstance().getRank(ConfigYML.kingname, resident.getNation()) == TownManager.getInstance().getRank(args[3], resident.getNation())) {
                ...
                if (TownManager.getInstance().getRank(args[3], resident.getNation()) != null) {
                    if (TownManager.getInstance().getResident(args[2]) == null || TownManager.getInstance().getResident(args[2]).getNation() == null) {
                        ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.playeroffline(resident.getPlayer()));
                        return;
                    }
                    if (TownManager.getInstance().getResident(args[2]).getNation() == resident.getNation()) {
                        if (!TownManager.getInstance().getResident(args[2]).hasranknation(TownManager.getInstance().getRank(args[3], resident.getNation()))) {
                            return;
                        }

                        Rank rank = TownManager.getInstance().getRank(args[3], resident.getNation());
                        TownManager.getInstance().getResident(args[2]).removeRankNation(rank);

                        for (String permission : rank.getPermissionList()) {
                            TownManager.getInstance().getResident(args[2]).addperm(permission);
                        }

                        ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankcombacksuc(resident.getPlayer()));
                    }
                } else {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.rankexist(resident.getPlayer()));
                }
            ...
            case "delete" -> {
                ...
                if (!resident.hasPerm("ASTRATOWN_NATION_RANK_DELETE")) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), Messages.permissionerror(resident.getPlayer()));
                    return;
                }
                if (TownManager.getInstance().getRank(args[2], resident.getNation()) == null) {
                ...
                TownManager.getInstance().removeRank(TownManager.getInstance().getRank(args[2], resident.getNation()));
            ...
            case "my" -> {
                List<Rank> ranks = resident.getNationRank();
                for (Rank rank : ranks) {
                    ChatUtil.sendSuccessNotification(resident.getPlayer(), rank.getName());
                }
            }
